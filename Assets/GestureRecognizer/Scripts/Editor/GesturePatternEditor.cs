	...
	public class GesturePatternEditor : Editor{
		...
		public override void OnInspectorGUI () {

			{
				...
				if (current != null && current.gesture != null && current.gesture.lines.Count > 0 && current.gesture != null && current.gesture.lines.Count == 0) {
					if (GUILayout.Button ("Convert to new GesturePattern",GUILayout.ExpandWidth(true),GUILayout.Height(100))) {
						...
						points.arraySize = current.gesture.lines.Count;

						//fill points
						for (int i = 0; i < current.gesture.lines.Count; i++) {
                            points.GetArrayElementAtIndex(i).vector2Value = current.gesture.lines[i];
						}
